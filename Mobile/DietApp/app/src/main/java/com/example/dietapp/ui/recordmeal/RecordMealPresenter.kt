...
) : BasePresenter<RecordMealView>() {

    fun getMeals() {
        mealsService.getUserMeals().enqueue(object : Callback<List<MealDTO>> {
            ...
            override fun onResponse(call: Call<List<MealDTO>>, response: Response<List<MealDTO>>) {
                ...
                mvpView?.replaceUserMeals(response.body()!!)
            }
        })
    }

    fun recordMeal(meal: RecordMealDTO) {
        mealEntriesService.recordMeal(meal).enqueue(object : Callback<List<MealEntryDTO>> {
            ...
            override fun onResponse(
                ...
                if (!response.isSuccessful) {
                    if (response.code() == 401) tryRefreshToken()
                    mvpView?.stopCpbAnimation(false)
                    return
                }
                mvpView?.stopCpbAnimation(true)
            }
